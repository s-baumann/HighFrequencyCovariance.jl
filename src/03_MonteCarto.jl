...
function generate_random_path(dimensions::Integer, ticks::Integer; syncronous::Bool = false, twister::MersenneTwister = MersenneTwister(1), minvol::Real = 0.0, maxvol::Real = 0.02,
                              min_refresh_rate::Real = 1.0, max_refresh_rate::Real = 5.0, min_noise_var::Real = 0.0, max_noise_var::Real = 0.01)
    wish = InverseWishart(dimensions, Matrix(Float64.(I(dimensions))))
    brownian_corr_matrix, _ = cov2cor(Hermitian(rand(twister, wish)))
    vols = minvol .+ (maxvol .- minvol) .* rand(twister, dimensions)
    assets = Symbol.(:asset_, 1:dimensions)
    ...
    micro_noise = min_noise_var .+ (max_noise_var .- min_noise_var) .* rand(twister, dimensions)
    microstructure_noise = Dict(assets .=> micro_noise)
