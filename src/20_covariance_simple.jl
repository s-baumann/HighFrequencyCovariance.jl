...
* time_grid::Union{Missing,Dict} - The grid with which to calculate returns.
* fixed_spacing::Union{Missing,Dict,<:Real} - A spacing used to calculate a time grid. Not used if refresh_times=true.
* refresh_times::Bool - Should refresh times be used to estimate covariance.
* rough_guess_number_of_intervals::Integer - A rough number of intervals to calculate a default spacing. Not used if a time_grid or fixed_spacing is provided or if refresh_times=true.
### Returns
* A `CovarianceMatrix`.
"""
