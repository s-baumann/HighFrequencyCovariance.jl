...
"""
function dataframe_to_covariancematrix(dd::DataFrame, error_if_incomplete::Bool = false)
                ...
                if nrow(c1) > 0
                    ...
                    if nrow(c2) == 0
                        if error_if_incomplete
                            error("The correlation between ", asseti, " and ", assetj, " is not in this dataframe.")
                        end
                        mat[i,j] = NaN
                    else
                        mat[i,j] = c2[1,:value]
